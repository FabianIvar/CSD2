# Minimum viable product -> deviation is not implemented yet
# Most functions have an 'enable_comments' variable which, when set to True, can give information about what's going on inside the function
...
def pack_sample_settings():
    """Starts sample settings configuration and saves the settings of each sample a nested dictionary"""
    ...
    if len(audiofile_names) < 1:
        print(f'\n{color.REDB}Error{color.RESET}, No valid files detected in working directory \n\n-> {color.YELLOWB}exiting program')
        exit()
    ...
    toggle_print(configuration, enable_comments)
...
def get_values_between(**arg): # arg is a dictionary containing the arguments used in this function
    ...
    parameter arg['stop']: The end value of the sequence (end value is included in list)
...
def duration_to_timestamp():
    ...
    for layer_name in sample_settings:
        ...
        # Add time_durations to the sample_settings dictionary
        sample_settings[layer_name]['time_durations'] = time_duration
...
def convert_to_events(input_dictionary, expected_length):
    """Converts input_dictionary to events"""
...
def prepare_for_event_conversion():
    """Prepares layers for being converted into events

    Reads out information from the packed_layers dictionary and creates and transforms dictionaries to ensure that
    the convert_to_events function has all the the required information for event conversion.

    What this function does:
        1) Get the value_list lengths for every layer of the key with the most values (usually the timestamps key),
            store these lengths in a list.
        2) The values in this list are the 'expected amount' -> check for every other key in the input
            dictionary if the amount of values matches the 'expected amount'
        3) If yes -> do nothing, if no -> add values to the key that was below the expected amount
            untill the amount of values reach the expected amount
    """

    enable_comments = False

    toggle_print('========[TRANSFORM LAYERS]========\n', enable_comments)
    ...
    # uses the layer_lengths of the key that has the most values -> length_keyname
...
prepared_dictionary = prepare_for_event_conversion()
# index 1 = expected amount of values of each key in the dict
expected_length = prepared_dictionary[1]
# index 0 = the prepared_dictionary
event = convert_to_events(prepared_dictionary[0], expected_length)
...
def player(time_zero):
    """Function that plays the generated events"""
    ...
    while pos < expected_length:
        ...
        # if there is no other channel available -> overwrite sound playing on assigned channel
...
# Player function returns channel -> handy for checking if sample has finished playing -> can be used to prevent a sudden click/cutoff
...
""" Sources
list filesnames:            https://stackoverflow.com/questions/3207219/how-do-i-list-all-files-of-a-directory#3207973
tell python to do nothing:  https://stackoverflow.com/questions/19632728/how-do-i-get-a-python-program-to-do-nothing#19632742
reading from .txt file:     https://www.geeksforgeeks.org/python/with-statement-in-python/#
try/except function:        https://www.w3schools.com/python/python_try_except.asp
indentifying variable type: https://www.w3schools.com/python/python_ref_string.asp
markdown formatting:        https://www.markdownguide.org/basic-syntax/
array methods:              https://www.w3schools.com/python/python_ref_list.asp
functions in python:        https://www.w3schools.com/python/python_functions.asp
for loop in python_basics:  https://www.w3schools.com/python/python_for_loops.asp
constrain function:         https://stackoverflow.com/questions/34837677/a-pythonic-way-to-write-a-constrain-function#34837691
dictionary's in python:     https://www.w3schools.com/python/python_dictionaries_access.asp
